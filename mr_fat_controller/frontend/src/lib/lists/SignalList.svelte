    ...
    <ul class="flex-1 overflow-auto space-y-1">
      {#each signals.data as signal}
        <li class="flex flex-row space-x-2 items-center py-1">
          {#if entitiesDict[signal.entity]}
            {#if activeState.signal[signal.id] && activeState.signal[signal.id].state === "off"}
              <Icon path={mdiCircleOutline} />
            {:else if activeState.signal[signal.id] && activeState.signal[signal.id].state === "danger"}
              <Icon path={mdiCircle} class="text-red-500" />
            {:else if activeState.signal[signal.id] && activeState.signal[signal.id].state === "clear"}
              <Icon path={mdiCircle} class="text-green-500" />
            {:else}
              <Icon path={mdiHelpRhombusOutline} />
            {/if}
            <span class="flex-1">{entitiesDict[signal.entity].name}</span>
          {/if}
        </li>
      {/each}
    </ul>
